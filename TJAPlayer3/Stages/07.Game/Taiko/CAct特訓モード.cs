    ...
    internal class CAct特訓モード : CActivity
    {
        ...
        public override int On進行描画()
        {
            if (!base.b活性化してない)
            {
                ...
                TJAPlayer3.act文字コンソール.tPrint(0, 0, C文字コンソール.Eフォント種別.白, "TRAINING MODE (UNSUPPORTED)");
                ...
                if (TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDXKeys.Key.LeftArrow) || TJAPlayer3.Pad.b押されたDGB(Eパッド.LBlue))
                {
                    if (this.b特訓PAUSE)
                    {
                        if (this.n現在の小節線 > 1)
                ...
                if (TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDXKeys.Key.RightArrow) || TJAPlayer3.Pad.b押されたDGB(Eパッド.RBlue))
                {
                    if (this.b特訓PAUSE)
                    {
