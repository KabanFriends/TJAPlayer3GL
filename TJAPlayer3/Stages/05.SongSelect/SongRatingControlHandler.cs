...
namespace TJAPlayer3
{
    internal static class SongRatingControlHandler
    {
        ...
        {
            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D0))
            {
                return SongRating.Unset;
            }

            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D1))
            {
                return SongRating.One;
            }

            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D2))
            {
                return SongRating.Two;
            }

            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D3))
            {
                return SongRating.Three;
            }

            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D4))
            {
                return SongRating.Four;
            }

            if (keyboard.bキーが押された((int) SlimDXKeys.Key.D5))
            {
